    ...
    try {
      await fetch(`https://formsubmit.co/ajax/${RECEIVER_EMAIL}`, {
        ...
        body: JSON.stringify({
          ...
          message,
          preferredType,
          preferredBatch: preferredType === "group" 
            ? preferredBatch 
            : preferredType === "pcod-pcos"
              ? "8:15 AM - 9:15 AM"
              : "N/A",
      ...
      {preferredType === "pcod-pcos" && (
        <div>
          ...
          <input
            id="pcodTiming"
            name="pcodTiming"
            value="8:15 AM - 9:15 AM"
            readOnly
            className="w-full rounded-xl border border-brand-cream-200 bg-brand-cream-50/50 px-4 py-3 text-sm text-brand-brown-400 outline-none opacity-80 cursor-not-allowed"
          />
        </div>
      )}
